(function() {
    ...
    ScheduleTemplate.prototype.openModal = function(target) {
        ...
        if (target.getAttribute("data-room") != '') {
         this.modalDate.textContent += ' | ' + target.getAttribute('data-room');
        }
        this.modal.setAttribute('data-event', target.getAttribute('data-event'));

        //update event content
        this.loadEventContent(target);
    ...
    ScheduleTemplate.prototype.loadEventContent = function(target) {
        ...
        httpRequest.onreadystatechange = function() {
            if (httpRequest.readyState === XMLHttpRequest.DONE) {
                if (httpRequest.status === 200) {
                    // self.modal.getElementsByClassName('cd-schedule-modal__event-info')[0].innerHTML = target.getAttribute('data-name') + self.getEventContent(httpRequest.responseText);
                    self.modal.getElementsByClassName('cd-schedule-modal__event-info')[0].innerHTML = self.getEventContentDetails(httpRequest.responseText, target);
    ...
    ScheduleTemplate.prototype.getEventContentDetails = function(string, target) {
        // reset the loaded event content so that it can be inserted in the modal
        var div = document.createElement('div');
        div.innerHTML = string.trim();
        var text = `<div>
			Durée: <strong> ${target.getAttribute("data-start")} - ${target.getAttribute("data-end")} </strong><br>
			Matière: <strong> ${target.getAttribute("data-name")} </strong><br>
			Enseignant: <strong> ${target.getAttribute("data-teacher")} </strong><br><br>
			Modalité: <strong>${target.getAttribute("data-modality")}</strong><br>`;

        if (target.getAttribute("data-room") != '') {
            text += `Salle: <strong>${target.getAttribute("data-room")}</strong><br>`;
        }
        if (target.getAttribute("data-floor") != '')
            text += `Etage: <strong>${target.getAttribute("data-floor")}</strong><br>`;
        if (target.getAttribute("data-campus") != '')
            text += `Campus: <strong>${target.getAttribute("data-campus")}</strong><br>`;
        text += `</div>`;
        div.getElementsByClassName('cd-schedule-modal__event-info')[0].innerHTML = text;
    ...
    ScheduleTemplate.prototype.getEventContent = function(string) {
        ...
        var div = document.createElement('div');
        div.innerHTML = string.trim();
        console.log(div.getElementsByClassName('cd-schedule-modal__event-info')[0].innerHTML)
        return div.getElementsByClassName('cd-schedule-modal__event-info')[0].innerHTML;
    };
